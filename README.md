...
> 2026-09-16 重新确定方向。Scriber 为候选名称；真实双路音频采集与混音已通过诊断验证，主面板接入和录屏仍在实现中。

## 原生应用开发

当前主面板支持麦克风录音、真实电平和 M4A 保存。新的 AudioRecorder 引擎支持电脑声音与麦克风单独或同时采集，并保存一份混合 M4A；已验证内置麦克风和 Jabra USB 的不同采样率。主面板双路控制和录屏将在后续 PR 接入。
