...
声音设备：两路声音旁显示设备名称，悬停可查看完整说明。系统默认输入／输出与设备可用状态持续刷新；录制中的麦克风名称对应本次采集绑定的设备。一路采集失败或超过一秒不再送出声音数据时，明确标记中断，保留另一条采集流和原声音选择；全部中断时安全结束。可先关闭失效来源，再开启重试；中断记录会随文件保留。自动重连和物理耳机切换仍待完成验收。
