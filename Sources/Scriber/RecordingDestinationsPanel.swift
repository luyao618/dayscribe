    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 18) {
            HStack(spacing: 8) {
                ...
                Text(L10n.text("保存位置")).font(.system(size: 18, weight: .semibold))
                Spacer()
            }
            Text(L10n.text("选好位置，每次录完自动保存。"))
                ...
                Button {
                    ...
                    VStack(alignment: .leading, spacing: 10) {
                        HStack(spacing: 8) {
                            ...
                            Text(L10n.text("\(mode.title)文件")).fontWeight(.semibold)
                            Spacer()
                        ...
                        Text(mode == .audio ? L10n.text("一份包含所选声音的音频文件") : L10n.text("视频和独立音频会放在一起"))
            ...
            Button(action: onBack) {
                Text(L10n.text("完成")).font(.system(size: 13, weight: .semibold))
                    .frame(maxWidth: .infinity).frame(height: 42)
                    .foregroundStyle(.white)
                    .background(PanelPalette.iris, in: RoundedRectangle(cornerRadius: 10))
            }
            .buttonStyle(.plain)
            Label(L10n.text("选择文件夹后自动记住，随时可以从 Finder 打开。"), systemImage: "folder")
                .font(.system(size: 10)).foregroundStyle(PanelPalette.slate)
                .fixedSize(horizontal: false, vertical: true)
        }
        .padding(20)
